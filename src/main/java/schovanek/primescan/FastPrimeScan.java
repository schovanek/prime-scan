...
public final class FastPrimeScan {
    ...
    public FastPrimeScan(PrintStream output, int dataColumIdx, int sheetIdx) {
    ...
    public void process(InputStream xlsxInputStream) throws IOException {
        ...
        try (ReadableWorkbook wb = new ReadableWorkbook(xlsxInputStream)) {
            ...
            try (Stream<Row> rows = sheet.openStream()) {
                rows.forEach(row -> {
                    String value = row.getCellText(dataColumIdx);
    ...
    private Thread getWorker(BlockingQueue<String> queue) {
        ...
        Thread worker = new Thread(primeCheckTask, "prime-check-worker");
        worker.start();
        return worker;
    }
